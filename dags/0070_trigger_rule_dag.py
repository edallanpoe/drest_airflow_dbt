...
def download_website_b():
    print("download_website_b {{ var.value.CASSANDRA_LOGIN }}")
    #raise ValueError("error")
...
with DAG(dag_id='dag0070_trigger_rule_dag', 
    ...
    bash_op = BashOperator(
        task_id="bash_op",
        bash_command="echo [{{ ds }}] {{ var.value.CASSANDRA_LOGIN }}",
    ...
    # ...
    download_website_a_task >> [ download_failed_task, download_succeed_task ]
    download_website_b_task >> [ download_failed_task, download_succeed_task ]
    (
        [ download_failed_task, download_succeed_task ]
            >> process_task
                >> [notif_a_task, notif_b_task] 
                    >> bash_op 
    )
    # download_failed_task >> process_task
    # download_succeed_task >> process_task
    # Both definitions declared above are the same.